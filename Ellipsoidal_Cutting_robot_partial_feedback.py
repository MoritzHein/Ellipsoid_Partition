...
stage_cost = ((x-x_ref).T@Q@(x-x_ref)) + u.T@R@u+ca.trace(0.5*Q@P_create)+ca.trace(0.5*R@K_create@P_create@K_create.T)**2
...
opt_x = ca_tools.struct_symSX([ca_tools.entry('x',repeat = [N+1], shape=(nx)),
                                 #ca_tools.entry('P_robot', repeat = [N+1], shape=(nx-2,nx-2)),
                                 #ca_tools.entry('P_hr', repeat = [N+1], shape=(nx-2,2)),
                                 ca_tools.entry('P', repeat = [N+1], shape=(nx,nx)),
                                 ca_tools.entry('beta', repeat = [N], shape=(nh_k)),
                                 ca_tools.entry('beta_N', shape=(nh_N)),
                                 ca_tools.entry('K', repeat = [N], shape=(6)),
                                 ca_tools.entry('u', repeat = [N], shape=(nu))])
opt_p = ca_tools.struct_symSX([ca_tools.entry('x_init', shape=(nx)),
                                 ca_tools.entry('P_human',repeat = [N+1], shape=(2,2))])
#%%
eps = 1e-5
J = 0
g = []
...
x_init = opt_p['x_init']
# g.append((opt_x['P_robot',0] - np.zeros((nx-2,nx-2))).reshape((-1,1)))
# lb_g.append(np.zeros((nx-2,nx-2)).reshape((-1,1)))
# ub_g.append(np.zeros((nx-2,nx-2)).reshape((-1,1)))
# g.append((opt_x['P_hr',0] - np.zeros((nx-2,2))).reshape((-1,1)))
# lb_g.append(np.zeros((nx-2,2)).reshape((-1,1)))
# ub_g.append(np.zeros((nx-2,2)).reshape((-1,1)))
...
for i in range(1):
    g.append((opt_x['K',i]-np.zeros((6,1))))
    lb_g.append(np.zeros((6,1)))
    ub_g.append(np.zeros((6,1)))
...
x_traj[0] += x_init[0]
x_traj[3]=v_ref
x_ref_traj.append(deepcopy(x_traj))
K_list = []
P_list = []
idx_l=[3,5,6]
for i in range(N+1):
    if i<N:
        K_opt = ca.SX.sym('K_opt',nu,nx)
        K_opt=0*K_opt
        for j in range(nu):
            for l in range(3):
                K_opt[j,idx_l[l]] += opt_x['K',i].reshape((2,3))[j,l]

        K_list.append(K_opt)
    # P_opt = ca.SX.sym('P_opt',nx,nx)
    # P_opt =0*P_opt
    # P_opt[0:5,0:5] += opt_x['P_robot',i]
    # P_opt[5:7,5:7] += opt_p['P_human',i]
    # P_opt[5:7,0:5] += opt_x['P_hr',i].T
    # P_opt[0:5,5:7] += opt_x['P_hr',i]
    # P_list.append(P_opt)
    P_list.append(opt_x['P',i])
    # g.append((opt_x['P_robot',i]).reshape((-1,1)))
    # lb_g.append(np.zeros((nx-2,nx-2)).reshape((-1,1)))
    # ub_g.append(ca.inf*np.ones((nx-2,nx-2)).reshape((-1,1)))
    # g.append((opt_x['P_hr',i]).reshape((-1,1)))
    # lb_g.append(np.zeros((nx-2,2)).reshape((-1,1)))
    # ub_g.append(ca.inf*np.ones((nx-2,2)).reshape((-1,1)))
    g.append((opt_x['P',i]).reshape((-1,1)))
    lb_g.append(np.zeros((nx,nx)).reshape((-1,1)))
    ub_g.append(ca.inf*np.ones((nx,nx)).reshape((-1,1)))
K_func = ca.Function('K_fun',[opt_x['K',-1]],[K_opt])
for i in range(N):
    # Cost
    J += stage_cost_fcn(opt_x['x',i], opt_x['u',i], P_list[i], K_list[i],x_traj)
    # Regularization
    J += 1e-2*ca.sumsqr(opt_x['K',i])
    ...
    if i >0:
        A_K = A_k + B_k@K_list[i]
    else:
        A_K =A_k
    # g.append((opt_x['P_robot',i+1] - ca.horzcat(ca.DM.eye(5), ca.DM.zeros((5,2)))@(A_K@P_list[i]@A_K.T + W_jac@W_cov@W_jac.T)@ca.horzcat(ca.DM.eye(5), ca.DM.zeros((5,2))).T).reshape((-1,1)))
    # lb_g.append(np.zeros((nx-2,nx-2)).reshape((-1,1)))
    # ub_g.append(np.zeros((nx-2,nx-2)).reshape((-1,1)))
    # g.append((opt_x['P_hr',i+1] - ca.horzcat(ca.DM.eye(5), ca.DM.zeros((5,2)))@(A_K@P_list[i]@A_K.T + W_jac@W_cov@W_jac.T)@ca.horzcat(ca.DM.zeros((2,5)), ca.DM.eye(2)).T).reshape((-1,1)))
    # lb_g.append(np.zeros((nx-2,2)).reshape((-1,1)))
    # ub_g.append(np.zeros((nx-2,2)).reshape((-1,1)))
    ...
    # Constraint
    if i == 0:
        g.append(-opt_x['beta',i] + H_fun(opt_x['x',i], opt_x['u',i], P_list[i], K_list[i]))
...
# Terminal cost
J += terminal_cost_fcn(opt_x['x',N], P_list[N],x_traj)
# Regularization of K

# Terminal constraint
g.append(opt_x['beta_N'] - HN_fun(opt_x['x',N], P_list[N]))
...
solver_opt = {'ipopt.max_iter':1500,'ipopt.linear_solver': 'MA27','ipopt.print_level':0, 'print_time':0, 'ipopt.tol':1e-8,'ipopt.alpha_min_frac':0.00001}
...
opt_x_init['x',:,1]=1
# opt_x_init['P_robot'] = 0.1
# opt_x_init['P_hr'] = 0.1
opt_x_init['P'] = np.zeros((nx,nx))+np.diag([0,0,0,0,0,0.4**2,0.4**2])
opt_x_init['beta'] = 1
opt_x_init['beta_N'] = 1
#opt_x_init['K'] = 0
opt_x_init['K']=K[:,[3,5,6]].reshape(-1)
opt_x_init['K']=0
# Set opt_p
opt_p_num = opt_p(0)
opt_p_num['x_init'] = x_init
opt_p_num['P_human',0] = np.zeros((2,2))
W_jac = W_func(np.zeros((nx,1)),np.zeros((nu,1)),np.zeros((nw,1)))
A_jac = A_func(np.zeros((nx,1)),np.zeros((nu,1)),np.zeros((nw,1)))
B_jac = B_func(np.zeros((nx,1)),np.zeros((nu,1)),np.zeros((nw,1)))
for i in range(N):
    opt_p_num['P_human',i+1] = A_jac[5:7,5:7]@opt_p_num['P_human',i]@A_jac[5:7,5:7].T+W_jac[5:7,:]@W_cov@W_jac[5:7,:].T

opt_x_init = np.load('nom_OL_sol.npy',allow_pickle=True).item()
res_nom = solver_eps(x0=opt_x_init,p=opt_p_num.cat,lbg=lb_g, ubg=ub_g)
#opt_x_num = opt_x(res_nom['x'])
opt_x_num_nom = opt_x(res_nom['x'])
print(res_nom['f'])
print(solver_eps.stats()['return_status'])
# First solve with all K=0, then save as 'nom_OL_sol.npy' to initialize the closed loop
#np.save('nom_OL_sol.npy',opt_x_num_nom,allow_pickle=True)
...
for j in range(50):
    ...
    for i in range(N_sim):
        opt_p_num['x_init'] = x_init
        res = solver_eps(x0=opt_x_init,p=opt_p_num,lbg=lb_g, ubg=ub_g)
        ...
        if sol_info[-1] == 'Solve_Succeeded' or sol_info[-1] == 'Solved_To_Acceptable_Level':
            ...
            opt_x_init = deepcopy(opt_x_feas)
            ...
            u0 += -K_func(opt_x_feas['K',ido])@(opt_x_feas['x',ido]-x_init)
        else:
            u0= np.array(opt_x_num['u',0]).reshape((nu,1))
        ...
        opt_x_init_prev = deepcopy(opt_x_init)
        # Time shift the previous solution
        for k in range(N):
            opt_x_init['x',k]=opt_x_init_prev['x',k+1]
            # opt_x_init['P_robot',k]=opt_x_num['P_robot',k+1]
            # opt_x_init['P_hr',k] = opt_x_num['P_hr',k+1]
            opt_x_init['P',k] = opt_x_init_prev['P',k+1]
            if k <N-1:
                opt_x_init['u',k]=opt_x_init_prev['u',k+1]
                if k>0:
                    opt_x_init['K',k]=opt_x_init_prev['K',k+1]
                opt_x_init['beta',k]=opt_x_init_prev['beta',k+1]
...
np.save('data_list_ol_PFB_final.npy',data_list_ol,allow_pickle=True)
# 
# %% Print mean CLC
data_list_ol = np.load('data_list_ol_PFB_final.npy',allow_pickle=True)
clc_ol = []
time_ol = []
...
fig.align_labels()
fig.tight_layout()
fig.savefig('sim_study_OL_pfb.pdf')
...
with plt.style.context(['science','ieee']):
    ...
    fig.align_labels()
    fig.tight_layout()
    fig.savefig('sim_study_OL_PFB_Phase.pdf')
# %% Plot the ellipsoids
idx_run = 0
idx_time = 0
...
for i in range(0,N+1):
    ...
    confidence_ellipse(np.array(opt_x_num_ell['x',i]), np.array(opt_x_num_ell['P',i]), ax[0,0],dim=[0,1], edgecolor='b')
...
ax.set_xlabel('$x_{r,x} \\text{ and } x_{h,x}$')
ax.set_ylabel('$x_{r,y} \\text{ and } x_{h,y}$')
X_traj = np.array(dat['X']).T
U_traj = np.array(dat['U']).reshape((-1,nu)).T
# Trajectory
...
fig.align_labels()
fig.tight_layout()
fig.savefig('Prediction_loop_OL_pfb.pdf')
...
opt_x = ca_tools.struct_symSX([ca_tools.entry('x',repeat = [N+1,ns], shape=(nx)),
                                 ...
                                 ca_tools.entry('K', repeat = [N,ns], shape=(6)),
...
for s in  range(ns):
    ...
    for i in range(1):
        g.append((opt_x['K',i,s]-np.zeros((6,1))))
        lb_g.append(np.zeros((6,1)))
        ub_g.append(np.zeros((6,1)))
x_traj = 0*x_init    
x_traj[0] += x_init[0]
...
for i in range(N+1):
    ...
    for s in range(ns):
        if i<N:
            ...
            for j in range(nu):
                for l in range(3):
                    K_opt[j,idx_l[l]] += opt_x['K',i,s].reshape((2,3))[j,l]
            K_sub_list.append(K_opt)
            # Constraint the traces to be greater than 0
            g.append(ca.trace(0.5*R@K_opt@opt_x['P',i,s]@K_opt.T))
            lb_g.append(np.zeros((1,1)))
            ub_g.append(ca.inf*np.ones((1,1)))
            

        g.append(opt_x['P',i,s].reshape((-1,1)))
        lb_g.append(np.zeros((nx,nx)).reshape((-1,1)))
        ub_g.append(ca.inf*np.ones((nx,nx)).reshape((-1,1)))
        # Constraint the traces to be greater than 0
        g.append(ca.trace(0.5*Q@opt_x['P',i,s]))
        lb_g.append(np.zeros((1,1)))
        ub_g.append(ca.inf*np.ones((1,1)))
    K_list.append(K_sub_list)
for i in range(N):
    for s in range(ns):
        ...
        if i == 0 and s == 0:
            ...
            B_k = B_func(opt_x['x',i,s], opt_x['u',i,s],np.zeros((nw,1)))
            W_jac = W_func(opt_x['x',i,s], opt_x['u',i,s],np.zeros((nw,1)))
            A_K = A_k + B_k@K_list[i][s]
            g.append((opt_x['P',i+1,s] - A_K@opt_x['P',i,s]@A_K.T - W_jac@W_cov@W_jac.T).reshape((-1,1)))
            lb_g.append(np.zeros((nx,nx)).reshape((-1,1)))
            ub_g.append(np.zeros((nx,nx)).reshape((-1,1)))

        g.append(opt_x['beta',i,s] - H_fun(opt_x['x',i,s], opt_x['u',i,s], opt_x['P',i,s], K_list[i][s]))
        lb_g.append(np.zeros((nh_k,1)))
        ...
        J += stage_cost_fcn(opt_x['x',i,s], opt_x['u',i,s], opt_x['P',i,s], K_list[i][s],x_traj)
        # Regularization of K
        J+= 1e-0*ca.sumsqr(opt_x['K',i,s])
...
solver_opt = {'ipopt.max_iter':3000,'ipopt.linear_solver': 'MA27','ipopt.print_level':0, 'print_time':0,'ipopt.alpha_min_frac':0.00001,'ipopt.acceptable_tol':1e-5,'ipopt.tol':1e-7}
...
opt_x_num_nom = np.load('nom_OL_sol.npy',allow_pickle=True).item()
...
opt_x_init['P',:,0] = [opt_x_num_nom['P',i] for i in range(N+1)]
...
opt_x_init=np.load('nom_CL_sol_fb0.npy',allow_pickle=True).item()
...
# Initialize again with K=0 and alpha=0 to generate the initial guess
#np.save('nom_CL_sol_fb0.npy',opt_x_num)
...
for j in range(50):
    ...
    clc = []
    sol_info = []
    # Initialize ellipsoid sampling
    #Decompose W_cov into axis and rotation
    d, V= np.linalg.eig(sigma_w**2*np.eye((nw*N_sim)))
    D = np.diag(d)
    opti_k= None
    ...
    for i in range(N_sim):
        ...
        if sol_info[-1] == 'Solve_Succeeded' or sol_info[-1] == 'Solved_To_Acceptable_Level':
            ...
            if select0:
                ...
                u0 += np.array(K_func(opt_x_feas['K',ido,0])@(x_init-x1))
                
            else:
                u0= np.array(opt_x_feas['u',ido,1]).reshape((nu,1))
                u0 += np.array(K_func(opt_x_feas['K',ido,1])@(x_init-x2))
        else:
            u0= np.array(opt_x_num['u',0,0]).reshape((nu,1))

        # Update opt_x_init by timeshifting
        opt_x_init_prev = deepcopy(opt_x_init)
        if ido>0:
            if select0:
                idx_scen=0
            ...
            for k in range(N):
                for s in range(ns):
                    if k< N-1:
                        opt_x_init['u',k,s] = opt_x_init_prev['u',k+1,idx_scen]
                        opt_x_init['K',k,s] = opt_x_init_prev['K',k+1,idx_scen]
                        opt_x_init['beta',k,s] = opt_x_init_prev['beta',k+1,idx_scen]
                    else:
                        opt_x_init['K',-1,s]=0
                    opt_x_init['x',k,s] = opt_x_init_prev['x',k+1,idx_scen]
                    opt_x_init['P',k,s] = opt_x_init_prev['P',k+1,idx_scen]
        ...
        x_ref[1:]=0
        x_ref[3] = v_ref
        clc.append(stage_cost_fcn(x_init, u0, np.zeros((nx,nx)), np.zeros((nu,nx)), x_ref))
        U.append(u0)
        p0= P[nw*i:nw*(i+1)]
...
np.save('data_list_pfb_final.npy',data_list,allow_pickle=True)

# %% Print mean clc
data_list=np.load('data_list_pfb_final.npy',allow_pickle=True)
...
fig.align_labels()
fig.tight_layout()
fig.savefig('sim_study_Part_pfb_Scen.pdf')
# %% Only first plot
with plt.style.context(['science','ieee']):
    l=0
    fig, ax = plt.subplots(1,1,figsize=(2,2)) 
    ...
    fig.align_labels()
    fig.tight_layout()
    fig.savefig('sim_study_Part_PFB_Phase.pdf')
# %% 
...
print('Solution:', da['sol_info'][idx_time])
...
a = opt_x_num['cutting_a']
y = (b - a[-2]*x)/a[-1]
ax.plot(x, y,linewidth=1, label='Partitioning plane')
#ax[0,0].set_xlim(-0.6, 2)
ax.set_ylim(-1.4, 1.4)
# Plot ellipsoid
A_k = A_func(opt_x_num['x',0,0], opt_x_num['u',0,0],np.zeros((nw,1)))
...
fig.align_labels()
fig.tight_layout()
fig.savefig('Prediction_loop_pfb_scen.pdf')
